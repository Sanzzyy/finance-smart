...
export const KemampuanBeliModal = ({ show, onClose, userId, saldo, rataPengeluaran }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/60 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl relative animate-in zoom-in-95 duration-200 border border-slate-100 dark:border-slate-700">
        <button
          onClick={onClose}
        ...
        {/* INFO SALDO CURRENT */}
        <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 mb-6">
          <div className="flex flex-col">
            <span className="text-[10px] uppercase font-black tracking-widest text-slate-400 mb-1">Saldo Saat Ini</span>
            <span className="font-bold text-slate-700 dark:text-slate-200">Rp {saldo?.toLocaleString("id-ID")}</span>
          </div>
          <div className="flex flex-col text-right">
            <span className="text-[10px] uppercase font-black tracking-widest text-slate-400 mb-1">Rata-rata Pengeluaran</span>
            <span className="font-bold text-slate-700 dark:text-slate-200">Rp {rataPengeluaran?.toLocaleString("id-ID")}</span>
          </div>
        </div>
        ...
        {result && (
          <div className={`p-5 rounded-2xl border animate-in slide-in-from-bottom-4 duration-300 ${result.status?.toLowerCase() === "bisa" ? "bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/30" : "bg-rose-50 dark:bg-rose-500/10 border-rose-200 dark:border-rose-500/30"}`}>
            ...
            <p className="text-sm font-medium leading-relaxed dark:text-slate-200 text-slate-700">
              {result.pesan}
            </p>
          </div>
        )}
